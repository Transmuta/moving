...
# Papel do membro (RBAC). Verificado em roleMeta [:2408] e no seed [:203]-[:207].
defmodule Movimento.Accounts.Role do
  use Ash.Type.Enum, values: [:admin, :profissional, :membro]
end
...
```elixir
# NAO-VERIFICADO: AshAuthentication (estratégia de senha, tokens) — confirmar contra hexdocs
...
Ações: `:register`, `:sign_in_with_password`, `:request_password_reset`, `:reset_password`
(delegadas à AshAuthentication). Casam com `POST /auth/sign_in`, `GET /auth/me` do `09` §3.10.
...
Corresponde a `membros` ([`:203`](../interface/Movimento.dc.html#L203)). É global porque
liga um `User` global a uma `Clinic`. O vínculo com um `Professional` (que vive **dentro**
do schema do tenant) é um UUID mole (`professional_id`), sem FK entre schemas.
...
defmodule Movimento.Accounts.Membership do
  ...
  attributes do
    uuid_primary_key :id
    attribute :papel, Movimento.Accounts.Role, allow_nil?: false, default: :membro
  ...
  actions do
    ...
    update :accept_invite     # pendente → ativo
...
policy real. `admin` é bypass; `membro` opera a agenda de todos, sem configurações; o
`profissional` é um **filtro** (`FilterCheck`) ao próprio.
...
# Appointment
policies do
  bypass actor_attribute_equals(:papel, :admin) do
    authorize_if always()
  end

  policy action_type(:read) do
    authorize_if actor_attribute_equals(:papel, :membro)   # opera a agenda de todos
    authorize_if Movimento.Checks.OwnAgenda                # profissional: só a própria
  end

  policy action_type([:create, :update, :destroy]) do
    authorize_if actor_attribute_equals(:papel, :membro)
...
(`ClinicHours`, `AppointmentType`, `Professional.create`, `Membership`) são `authorize_if
actor_attribute_equals(:papel, :admin)` — o "sem configurações sensíveis" do `membro`
