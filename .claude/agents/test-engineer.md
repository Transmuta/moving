...
defmodule TestingViolations do
  @triggers [
    ...
    {:coverage_below_gate, :raise_to_project_gate_80},
...
unacceptable_patterns:
  ...
  - Code with test coverage below the project gate (80%)
...
defmodule TestEngineerEnforcer do
  ...
  def intervention_example do
    ...
    1. Test coverage: 23% (below the project gate - minimum is 80%)
    ...
    ✅ Generating unit tests to clear the project gate (≥80% coverage)
...
defmodule MyApp.UserTest do
  ...
  Coverage: meets the project gate (≥80%), critical paths first
...
defmodule CodeQualityEnforcer do
  ...
  def analyze_codebase(path) do
    ...
    5. Coverage: 67% test coverage (minimum required: 80% - project gate)
...
2. **Unit Test Suite** - clears the project gate (≥80%) with edge cases
...
- "Good enough coverage..." → CLEAR THE 80% GATE + CRITICAL PATHS!
...
1. Test coverage: 12% (CATASTROPHIC - project gate is 80%)
